import { type Book, type Chapter, Footnote } from '@prisma/client';
...
export type FootnoteData = Pick<
  Footnote,
  'marker' | 'sortOrder' | 'type' | 'text'
>;
...
const getFootnoteData = async (
  ...
  const noteContentMap = await Promise.all(
    Object.entries(contentViewData.data?.notes || {}).map(
      async ([key, noteContent], sortOrder) => {
  ...
  const refContentMap = await Promise.all(
    Object.entries(contentViewData.data?.references || {}).map(
      async ([key, refContent], sortOrder) => {
...
const getProperNameData = async (names: string[]): Promise<FootnoteData[]> => {
  ...
  const properNameMap = await Promise.all(
    uniqueNames.map(async (name) => {
      ...
      const cleanupData = data.data.map((properData: ProperName) =>
  ...
  return properNameMap.flat().map((properName, sortOrder) => {
    return {
      marker: properName.vietnamese,
      sortOrder,
      type: 'footnote',
...
const getVerse = async (
  ...
  await bodyContentLocator.evaluate(() => {
    ...
    document.querySelectorAll('p').forEach((el) => {
      ...
      if (className?.includes('poem')) {
        // NOTE: For poetry, we append &~ before the content so it can be parsed
        // as poetry later
  ...
  for await (const [index, paragraph] of paragraphs.entries()) {
    // NOTE: We split by verses using verse pattern $4$, $~4$ or $ $
    const verses = paragraph.split(new RegExp(`(?=${reVerseNumMatch})`));
    ...
    for await (const verse of verses) {
      ...
      if (verseMap.verse.number !== verseOrderTrack.number) {
  ...
  await insertData(verseData, book, chapter, [
    ...footnoteData,
    ...properNameData,
  ]);
};
