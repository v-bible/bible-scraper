...
import {
  ...
  mark,
  verse,
} from '@/../drizzle/sqlite/schema.js';
import { type DrizzleClient } from '@/drizzle/sqlite/index.js';

// Upsert operations for Drizzle SQLite
...
export async function upsertVerse(
  ...
  await db.insert(verse).values(newVerse);
  return newVerse as Verse;
}
...
export async function upsertHeading(
  ...
  await db.insert(heading).values(newHeading);
  return newHeading as Heading;
}
